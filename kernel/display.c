...
static void bga_internal_set_resolution(uint16_t width, uint16_t height)
{
    ...
    bga_internal_write_reg(VBE_DISPI_INDEX_ENABLE, VBE_DISPI_ENABLED | VBE_DISPI_LFB_ENABLED);
...
int bga_ioctl(uint32_t cmd, uint32_t arg) {
    ...
    switch (cmd) {
        ...
        case BGA_SWAP_BUFFERS:
            ...
            bga_internal_write_reg(VBE_DISPI_INDEX_Y_OFFSET, (uint16_t)y_offset);
            return 0;
        case BGA_DISABLE:
            bga_internal_write_reg(VBE_DISPI_INDEX_ENABLE, VBE_DISPI_DISABLED);
            return 0;
...
int init_display(pci_dev_t *dev) {
    ...
    kinfo("bga mapped in kernel cr3=%p\n", new_cr3);
