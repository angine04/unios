 ...
 * \return bar
 */
static uint32_t pci_internal_read_bar(
    uint8_t bus, uint8_t device, uint8_t function, uint8_t bar_id) {
...
static bar_t
    pci_get_bar(uint8_t bus, uint8_t device, uint8_t function, uint8_t bar_id) {
    bar_t result;
...
#ifdef DEBUG_PCI
            kinfo(
                "PCI: DEVICE_UNKNOWN: Class %d subclass %d",
                dd->class_id,
                dd->subclass_id);
#endif
    ...
    for (bus = 0; bus < 8; bus++) {
        for (device = 0; device < 32; device++) {
            ...
            for (function = 0; function < functions_count; function++) {
                pci_dev_t dev = pci_get_device_desriptor(bus, device, function);
...
#ifdef DEBUG_PCI
                kinfo(
                    "PCI: Vendor %x, devID %x, class %x subclass %x\n",
                    dev.vendor_id,
                    dev.device_id,
                    dev.class_id,
                    dev.subclass_id);
...
uint32_t pci_read_bar(pci_dev_t* pci_dev, int bar_id) {
    return pci_internal_read_bar(
        pci_dev->bus, pci_dev->device, pci_dev->function, bar_id);
}
...
pci_dev_t* get_pci_bga() {
    ...
    for (int i = 0; i < NR_PCI_DEV; ++i) {
        if (pci_devs[i].type == DEVICE_DISPLAY) {
            ...
            kinfo(
                "vendor: %x, device: %x\n",
                pci_devs[i].vendor_id,
                pci_devs[i].device_id);
            return &pci_devs[i];
        }
    }
